(function (global) {
  ...
  async function fileDigest(file){if(!file)return null;const bytes=await file.arrayBuffer();const digest=await crypto.subtle.digest('SHA-256',bytes);return Array.from(new Uint8Array(digest)).map(b=>b.toString(16).padStart(2,'0')).join('')}
  async function campaignFingerprint(){
    const creator=$('creator')?.files?.[0]||null;
    const product=$('product')?.files?.[0]||null;
    const category=normalize($('category')?.value);
    const restoredCategory=normalize(state.restoredIdentity?.category);

    if(!creator&&!product&&state.currentFingerprint&&state.restoredIdentity&&category===restoredCategory)return state.currentFingerprint;

    const creatorDigest=await fileDigest(creator);
    const productDigest=await fileDigest(product);
    if(!state.currentCampaignId&&(!creatorDigest||!productDigest))throw new Error('Upload one creator reference and one product reference to start your free campaign.');

    const bytes=new TextEncoder().encode([
      creatorDigest||`changed-from:${state.currentFingerprint||'creator'}`,
      productDigest||`changed-from:${state.currentFingerprint||'product'}`,
      category
    ].join('|'));
  ...
  function injectUI(){
    ...
    bar.innerHTML='<span><strong>Campaign access</strong> · revisions stay in the same campaign</span><span id="campaignUsageState">Checking access…</span>';
  ...
  function renderUsage(){
    const label=$('campaignUsageState');if(!label)return;
    const context=state.context||{};
    if(context.paid){label.textContent=context.founder_number?`Lifetime unlimited · Founding 100 #${String(context.founder_number).padStart(3,'0')}`:'Lifetime unlimited';return;}
    label.textContent=context.used?'Free campaign active · creator + product + category locked':'1 free campaign · unlimited revisions';
  }
  ...
  async function createCampaign(fingerprint,variables){const sb=client();const{data,error}=await sb.rpc('create_campaign_if_allowed',{p_title:variables.product_name||'Untitled campaign',p_product_name:variables.product_name||'Untitled product',p_product_fingerprint:fingerprint,p_product_variables:variables});if(error)throw error;state.currentCampaignId=data.id;state.currentFingerprint=fingerprint;state.restoredIdentity={category:variables.category||''};return data}
  async function recordRevision(variables,output){if(!state.currentCampaignId)return;const sb=client();const{error}=await sb.rpc('record_campaign_revision',{p_campaign_id:state.currentCampaignId,p_variables:variables,p_output:output});if(error)throw error;state.restoredIdentity={category:variables.category||''};const note=$('campaignRestoreState');if(note)note.textContent=`Saved: ${variables.product_name||'current campaign'}`}

  function isFreeCampaignLockError(error){return /free_campaign_locked/i.test(String(error?.message||error||''))}
  async function runLocalBuild(){
    ...
    try{
      ...
      if(isFreeCampaignLockError(error)){
        alert('Your free campaign is locked to the same creator, product and product category. Upgrade to lifetime access to start a different campaign.');
        try{parentWindow.dispatchEvent(new CustomEvent('aiugc:upgrade-required',{detail:{reason:'new_campaign'}}))}catch(_){ }
      }else{
  ...
  async function boot(){resetAutoOutput();injectUI();attachBuildGate();attachCopyGate();try{await loadContext();await restoreLatestCampaign();renderUsage()}catch(error){console.warn(error?.message||error);const label=$('campaignUsageState');if(label)label.textContent='Sign in required';const note=$('campaignRestoreState');if(note)note.textContent='Saved campaign unavailable.'}}

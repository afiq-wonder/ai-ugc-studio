(function (global) {
  ...
  function assertProvider(candidate) {
    if (!candidate || typeof candidate !== 'object') throw new TypeError('AI provider must be an object.');
    const supported = ['generateText', 'analyzeImages', 'analyzeDiscovery', 'generateCampaign'];
    if (!supported.some(method => typeof candidate[method] === 'function')) throw new TypeError('AI provider must implement a supported generation method.');
  }
  ...
  function selectedCategory(context) {
    return String(context?.input?.category || '').trim().toLowerCase();
  }

  function isClothingProduct(context) {
    if (/^(apparel|clothing|fashion|garment|wearable)$/.test(selectedCategory(context))) return true;
    return /\b(shirt|t[- ]?shirt|tee|jersey|baju|dress|gown|abaya|hijab|tudung|pants|trousers|jeans|skirt|blouse|top|jacket|hoodie|sweater|polo|kurung|kebaya|cardigan|clothing|apparel|garment|wear)\b/i.test(productName(context));
  }
  ...
  function wardrobeLock(context) {
    if (isClothingProduct(context)) {
      return `Preserve the creator's recognizable identity as the highest-priority visual invariant: face geometry, eyes, nose, mouth, skin tone, hairline/hair, facial hair, glasses and body proportions must continue to read as the same person. The promoted clothing becomes the wardrobe anchor for this scene. Transfer the garment onto this creator while preserving the product's visible design, silhouette, colorway, collar/neckline, sleeve treatment, piping/stripes, graphics, logos/text placement and recognisable details. Allow realistic drape, folds, stretch, tension and movement so the clothing behaves like real fabric on this creator's body. Do not reshape or reinterpret the creator to match the garment reference.`;
    }
  ...
  function placementLock(context) {
    ...
    if (isClothingProduct(context)) {
      return `APPAREL ROUTE: the creator reference controls WHO the person is and the product reference controls WHAT garment is worn. Keep creator identity immutable while transferring only the garment. The creator should wear the promoted garment naturally. Preserve the intended silhouette and fit while allowing the garment to respond realistically to pose and movement. Show enough of the garment for its key visual details and overall fit to remain commercially readable. Never derive face, body identity, pose, background or unrelated human features from the apparel reference.`;
    }
  ...
  function applyLocalDirectorFramework(context) {
    ...
    const route = isClothingProduct(context) ? 'APPAREL' : 'GENERAL';
  ...
  const adapter = {
    ...
    getProviderInfo() {
      ...
      return { configured: true, name: provider.name || 'custom', model: provider.model || null };
    },
    ...
    async enhanceCampaign(context) {
      if (!provider || typeof provider.generateCampaign !== 'function') return applyLocalDirectorFramework(context);
  ...
  loadScript('./precision-digital-compositor.js');

/* AI UGC Studio — Mini UGC Director v1.2
   Character Lock + Product Separation + Framing Guard
...
(function (global) {
  ...
  const libraries = {
    fashion: {
      focuses: [
        ...
        'sleeve, neckline, and fabric movement',
        'side profile and garment proportions',
        'material texture and construction details',
        'complete styled look in motion'
      ],
      actions: [
    ...
    beauty: {
      ...
      actions: [
        'brings the product naturally into frame while keeping the face readable',
        'dispenses a small amount and shows texture while the creator remains identifiable',
        'applies the product naturally while explaining one benefit',
        'shows the finished result from a realistic angle with the face clearly visible',
        'places the product back into the routine and gives a relaxed recommendation'
      ],
      shots: [
        'medium close-up with full head visible',
        'face-and-product framing with a gentle forward push',
        'face-and-hands framing with steady handheld movement',
        'close result framing that still preserves the full recognizable face',
        'medium lifestyle framing with subtle natural sway'
      ]
    },
    home: {
      ...
      actions: [
        'shows the everyday problem first while remaining identifiable, then brings the product into frame',
        'uses the product naturally to demonstrate its main function',
        'shows the key feature close to camera without hiding the creator for the whole scene',
        'reveals the finished result and compares it to the starting situation',
        'uses the product once more in a normal daily-life moment'
      ],
      shots: [
        'medium contextual creator shot with full head visible',
        'medium demonstration shot with a gentle forward push',
        'creator-established feature shot; brief detail close-up only',
        'wider result shot that keeps the creator identifiable',
        'lifestyle follow shot with subtle movement'
      ]
    },
    general: {
      ...
      actions: [
        'introduces the product immediately with a natural first reaction while facing the camera',
        'demonstrates the main feature with relaxed hand movement while keeping the face visible',
        'uses the product naturally and shows the main benefit in context',
        'brings one product detail closer to camera briefly without blocking the creator for the whole scene',
        'shows the final result and ends with a relaxed recommendation to camera'
      ],
      shots: [
        'medium chest-up creator framing with full head visible',
        'waist-up framing with full head visible and a gentle forward push',
        'medium-wide contextual framing with creator clearly identifiable',
        'creator-established detail framing; return to face after close-up',
        'medium creator framing with natural handheld follow'
      ]
    }
  };
  ...
  function productLabel(d) {
    return (d && d.product) || 'the product shown in the second reference image';
  }

  function sellingPoints(d) {
    return (d && d.sellingPoints) || 'quality, usefulness, comfort, and value';
  }
  ...
  function endingFor(index,total,seed) {
    const endings = [
      'briefly looks at the product, then back to camera before the cut',
      'releases the product naturally and looks back to camera',
      'changes stance while keeping both face and product clearly visible',
      'finishes the detail demonstration and returns attention to camera',
      'ends with a small confident smile and natural movement out of the shot'
    ];
  ...
  function direct(d,seed=0) {
    ...
    const roles = structures[d.style] || structures['Authentic UGC review'];
    const scenes = [];
    for (let i=0;i<d.scenes;i++) {
      const offset=(i+seed)%lib.actions.length;
      scenes.push({
        id:i,
        title:`Scene ${i+1} — ${roles[i] || 'Campaign Scene'}`,
        purpose:roles[i] || 'Advance the campaign naturally',
        category,
        productFocus:lib.focuses[offset],
        action:lib.actions[offset],
        camera:lib.shots[offset],
        dialogueIntent:dialogueIntent(roles[i] || '',i,d.scenes),
        ending:endingFor(i,d.scenes,seed)
      });
    }
    return scenes;
  }
...
AVOID:
identity drift, random casting, face replacement, hair/headwear substitution, neck-down-only framing, product distortion, extra fingers, warped hands, altered logos, fake text, duplicate objects, exaggerated expressions, repeated poses, repeated dialogue, or overly cinematic advertising.`;
  }
  ...
  function quality(scenes) {
    ...
    for(let i=0;i<scenes.length;i++) {
      for(let j=i+1;j<scenes.length;j++) {
        ['action','camera','productFocus','dialogueIntent'].forEach(k=>{if(scenes[i][k]===scenes[j][k]) duplicates++;});
      }
    }
  ...
  const Director = {
    ...
    detectCategory,
    direct,
    redirect(d,scene,attempt){return direct(d,attempt+1)[scene.id];},
    quality,
    identityPrompt,
    productPrompt,
    framingGuard,
